...
def write_or_update_report_markdown_with_data_validation(
    *,
    ...
    if DATA_VALIDATION_SECTION_START in current and DATA_VALIDATION_SECTION_END in current:
        start_idx = current.index(DATA_VALIDATION_SECTION_START)
        end_idx = current.index(DATA_VALIDATION_SECTION_END) + len(DATA_VALIDATION_SECTION_END)
